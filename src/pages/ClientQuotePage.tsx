  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingQuoteRequestId) {
        ...
        const { data: quotesToExpire, error: quotesCheckError } = await supabase
          .from('quotes')
          .select('id')
          .eq('quote_request_id', editingQuoteRequestId)
          .in('status', ['pending', 'accepted']);

        if (!quotesCheckError && quotesToExpire && quotesToExpire.length > 0) {
          ...
          if (quotesData && quotesData.length > 0) {
            ...
            if (moversData && moversData.length > 0) {
              const notifications = moversData.map(mover => ({
                ...
                message: 'Le client a modifié sa demande de déménagement. Veuillez vérifier et ajuster votre devis si nécessaire.',
                type: 'quote_update',
